...
export const SET_HOVERED_ADVANCED_COMPONENT = 'SET_HOVERED_ADVANCED_COMPONENT';
...
export const LOOP_BEGIN_ADVANCED = 'LOOP_BEGIN_ADVANCED';
export function loopBeginAdvanced() {
  return function (dispatch, getState) {
    const { views, mousePos, mode } = getState();
    if (mode.type === MODES.selectOrMove) { // only hover highlight in move mode
      dispatch({
        type: SET_HOVERED_ADVANCED_COMPONENT,
        views,
        mousePos,
        mode
      });
...
export const UPDATE_NETLIST = 'UPDATE_NETLIST';
export function updateNetlist (views) {
  return {
    type: UPDATE_NETLIST,
    views
  };
