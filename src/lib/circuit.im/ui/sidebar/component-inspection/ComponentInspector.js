...
import UploadFileSelector from './editable-types/FileUpload';
import Selector from './editable-types/Selector'
...
class ComponentInspector extends React.Component {
  ...
  render() {
    ...
    return (
      <div>
        <div style={styles.title}>Circuit Configuration</div>
        <hr />
        {(() => {
          if (selectedComponent) {
            ...
            const editComponents = editableNames.map(editableName => {
              ...
              switch (editablesSchema[editableName].type) {
                ...
                case 'file': {
                  EditComponent = UploadFileSelector;
                  break;
                }
                case 'radio': {
                  EditComponent = Selector;
                  break;
                }
                default:
                  return null;
              ...
              return (
                <EditComponent
                  ...
                  options={editablesSchema[editableName].options}
                />
              );
            });
            ...
            return (
              <div>
                <div style={styles.title}>No component selected</div>
                <hr />
                <div style={R.merge(style, styles.container)}>
                  <div style={R.merge(styles.outerBase, styles.content.outer)}>
                    <div style= {{ 'color': '#ddd'}}>
                      {camelToSpace('NetlistFile')}
                    </div>
                    <input name="myFile" type="file" style={{width:'inherit'}}/>
                  </div>
                </div>
              </div>
            );
