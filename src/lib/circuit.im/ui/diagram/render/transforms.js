...
const centerMidAdvanced = (ctx, props, next) => {
  const { dragPoints: [mid] } = props;
...
export default {
  ...
  3: {
    transformCanvas: centerMidAdvanced,
    getTransformedConnectors(dragPoints) {
      return [{ x: -20 - 20, y: 0 }, { x: 20 + 20, y: 0 }, { x: 0, y: 20 + 20 }];
    },
    getConnectors(dragPoints) {
      let x0 = dragPoints[0].x;
      let y0 = dragPoints[0].y;
      return [{ x: x0 -20 - 20, y: y0 }, { x: x0 + 20 + 20, y: y0 }, { x: x0, y: y0 + 20 + 20 }];
    }
  },
  4: {
    transformCanvas: centerMidAdvanced,
    getTransformedConnectors(dragPoints) {
      return [{ x: -20 - 20, y: 0 }, { x: 20 + 20, y: 0 }, { x: 0, y: 20 + 20 }, { x: 0, y: -20 - 20 }];
    },
    getConnectors(dragPoints) {
      let x0 = dragPoints[0].x;
      let y0 = dragPoints[0].y;
      return [{ x: x0 - 20 - 20, y: y0 }, { x: x0 + 20 + 20, y: y0 }, { x: x0, y: y0 + 20 + 20 }, { x: x0, y: y0 - 20 - 20 }];
    }
  },
  identity: {
